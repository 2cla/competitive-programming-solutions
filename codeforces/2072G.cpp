...
void solve(){
    ...
    ll q=p-1;
    while(q>0){
        ll lo=n/(q+1)+1,hi=n/q;
        lo=max(lo,q+1);hi=min(hi,k);
        if(hi<=q||lo>hi||n/lo!=q){
            q--;continue;
        }
        ll kk=hi-lo+1;
        ll d=n%hi;
        ans+=kk*q;
        ans+=mul(mul(kk,d),hi);
        ans+=mul((q*hi-d)%mod,((kk*(kk-1))/2)%mod);
        ans+=mod-mul(q,(((kk*(kk-1)*(2*kk-1))/6)%mod));
        ans%=mod;
        q--;
    }
    ans=add(ans,mul(n,max(0ll,k-n)%mod));
    cout<<ans<<'\n';
}
int main(){
